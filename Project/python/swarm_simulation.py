...
class World():
    def __init__(self):
        ...
        # Create the plane.
        self.planeId = p.loadURDF(os.path.join(os.path.dirname(__file__),"../models/plane.urdf"))
        p.changeDynamics(self.planeId, -1, lateralFriction=5., rollingFriction=0)

        self.goalId = p.loadURDF(os.path.join(os.path.dirname(__file__),"../models/goal.urdf"))
        
        p.resetDebugVisualizerCamera(7.0,50.0, -35.0, (1., 1., 0.0))
        
        # Add objects
        wallId = p.loadSDF(os.path.join(os.path.dirname(__file__),"../models/walls.sdf"))[0]
